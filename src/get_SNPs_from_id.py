...
def main():
    ...
    tissue_list = ['pancreas'] #['pancreas', 'spleen', 'adipose_tissue', 'adrenal_gland', 'pituitary', 'thyroid', 'blood', 'brain', 'heart', 'kidney', 'liver', 'muscle', 'nerve', 'lung']

    tissue_list.sort()

    for tissue in tissue_list:

        fba_filter_res = pd.read_table('C:/Users/Sigve/Genome_Data/results/ind_results/filtered/start_stop_het/ind_results_f_{0}_selected.tsv'.format(tissue), index_col=0)
        ...
        fba_filter_res[['sample_ids', 'solution', 'tasks_results', 'variants']].to_csv(path_or_buf='C:/Users/Sigve/Genome_Data/results/ind_results/filtered/start_stop_het/ind_results_f_{0}_selected.tsv'.format(tissue), sep='\t')
